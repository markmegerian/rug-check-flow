...
describe("reminder workflow correctness", () => {
  ...
  it("skips estimate creation and queueing for cleaning-only check-ins", () => {
    const edge = readFileSync(
      resolve(process.cwd(), "supabase/functions/check-in-workflow/index.ts"),
      "utf-8",
    );
    expect(edge).toContain("return Boolean(rule?.requires_estimate) && !isCleaningCategory(rule?.category);");
  });
});
