...
as $$
  ...
  approved_non_cleaning_services as (
    select
      rs.rug_id,
      rs.service_name,
      rs.line_total,
    ...
    where coalesce(lower(trim(rs.service_category)), '') <> 'cleaning'
      and coalesce(rs.approval_status, 'pending') = 'approved'
  ),
  rug_service_rollup as (
    ...
    from approved_non_cleaning_services rs
    group by rs.rug_id
  )
  select
    r.id,
  ...
  from public.rugs r
  join rug_service_rollup rsr on rsr.rug_id = r.id
  left join public.clients c on c.id = r.client_id
  left join active_delivery_allocations ada on ada.rug_id = r.id
  where r.status in ('checked_in', 'in_production')
