...
login_file="$(mktemp)"
login_status="$(curl -sS -o "$login_file" -w '%{http_code}' -X POST "${SUPABASE_URL}/auth/v1/token?grant_type=password" -H "apikey: ${SUPABASE_ANON_KEY}" -H "Content-Type: application/json" -d "$(json_payload "$OFFICE_USER_EMAIL" "$OFFICE_USER_PASSWORD")")"
...
results_file="$(mktemp)"
{
  ...
  http_check "fn.invoice-pdf" "${SUPABASE_URL}/functions/v1/invoice-pdf" "POST" "$(python - "$SAMPLE_INVOICE_ID" <<'PY'
import json,sys
